 ...
 */
public class BeanMenuPagina {

    public static String seleccionar = "select id_pag, nom_pag, url, nivel_pag from pagina WHERE nivel_pag = 1";
    private int id_pag;
    ...
    public List<BeanMenuPagina> getListado() {
        List<BeanMenuPagina> lista = new ArrayList<BeanMenuPagina>(0);
        try {
            Connection cnn = cConnection.conectar_ds();
            ResultSet rset = null;
            Statement sta = cnn.createStatement();
            rset = sta.executeQuery(BeanMenuPagina.seleccionar);

            while (rset.next()) {
                BeanMenuPagina current = new BeanMenuPagina();
